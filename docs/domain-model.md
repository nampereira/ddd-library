...
| **Security**    | Manages user identity, authentication, and role-based access |
...
| **UserId** | Value Object | A UUID referencing a user. Extracted from the JWT token at the web layer and passed into the domain without any coupling to the Security area. The domain never loads the `User` entity. |
...
| **Value Object** | An immutable domain concept defined entirely by its value, not by identity. Created with validated constructors so invalid values can never be constructed (`Isbn`, `BarCode`, `ContactEmail`, `BookTitle`, `LoanId`, `UserId`). |
