...
| **Spring Modulith**       | ~~Removed~~ — module boundaries are enforced by convention and code review, not by framework tooling |
...
Detailed write-ups for each use case live in `docs/`. Each document covers requirements, domain model, flow (text + sequence diagram), architecture class diagram, code patterns, and concurrency considerations.
...
LoanBookUseCase
    │
    └── LoanBookService.loan(barCode, userId)
            ├── LoanRepository.isAvailable(barCode)   ← database query
            └── new Loan(barCode, userId)             ← registers LoanCreated event
```
...
graph TD
    ...
    LENDING -->|"LoanCreated / LoanClosed"| CATALOG
    LENDING -->|"reads userId from JWT"| SECURITY
    CATALOG -->|"reads copyId from event"| SECURITY
```

**General principle:** modules communicate via shared value objects (`BarCode`) and, for extensibility, domain events (`LoanCreated`, `LoanClosed`). The Catalog module does NOT know about `Loan`. Copy availability is not managed via events — it is always queried directly from `LoanRepository.isAvailable(barCode)` in the lending context.
...
| **Functional Area**          | A logical boundary in which domain terms have a specific meaning; this project has three: security, catalog, and lending |
