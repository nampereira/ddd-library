 ...
 * {@link Authentication#getPrincipal()}, which holds the patron UUID as a {@code String}.</p>
 *
...
public class LendingController {
    ...
    public ResponseEntity<EntityModel<LoanResponse>> loan(@Valid @RequestBody LoanRequest request, Authentication auth) {
        UUID patronUuid = UUID.fromString((String) auth.getPrincipal());
        Loan loan = loanBook.execute(request.barCode(), new PatronId(patronUuid));
