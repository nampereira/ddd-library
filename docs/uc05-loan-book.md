...
- The patron's identity (`PatronId`) is derived from the JWT token; the controller does not accept a patron ID in the request body. Patron identities are managed by the security context and extracted transparently from the JWT.
...
| Value Object | `PatronId` | Lending | Identity of the borrowing patron |
