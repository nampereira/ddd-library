...
| **Optimistic Locking** | `Loan` carries a `@Version` field. If two transactions try to modify the same loan concurrently, one will be rejected with an `ObjectOptimisticLockingFailureException` rather than silently overwriting. |
